// pages/IncomingFormPage.tsx (Versi Perbaikan Final)
...
}> = ({ setColor, setLineWidth, setMode, clearCanvas, currentColor, currentWidth, currentMode }) => {
...
const IncomingFormPage: React.FC = () => {
  ...
  const [successInfo, setSuccessInfo] = useState<SuccessInfo | null>(null); // <-- State baru
  ...
  useEffect(() => {
    ...
    if (!canvas || !image || !transaction?.item.photoUrl) return;
    ...
    image.src = transaction.item.photoUrl;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (latestReturn?.id) {
        // Alih-alih redirect, kita set state successInfo
        const fullApprovalLink = `${window.location.origin}/#${AppRoutes.APPROVAL}/${transaction.id}?type=incoming&returnId=${latestReturn.id}`;
        setSuccessInfo({
            message: `Return for transaction #${transaction.transactionNumber} has been recorded.`,
            approvalLink: fullApprovalLink,
            printLink: `${AppRoutes.PRINT_INCOMING}/${transaction.id}/${latestReturn.id}`
        });
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-xl">
      ...
      <div className="mb-6">
        ...
        <DrawingControls setColor={(c) => setDrawConfig(p => ({...p, color: c}))} setLineWidth={(w) => setDrawConfig(p => ({...p, lineWidth: w}))} setMode={(m) => setDrawConfig(p => ({...p, mode: m}))} clearCanvas={handleClearCanvas} currentColor={drawConfig.color} currentWidth={drawConfig.lineWidth} currentMode={drawConfig.mode} />
      </div>
